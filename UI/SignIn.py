...
class SignInWidget(QWidget):
    ...
    def login_in_callback(self, str):
        print('callback : %s' % str)

    def signInCheck(self):
        adminId = self.lineEdit1.text()
        adminPasswd = self.lineEdit2.text()
        if (adminId == "" or adminPasswd == ""):
        ...
        # 组装json数据
        data = {'messageId' : 1001, 'adminId' : adminId, 'adminPasswd' : adminPasswd, 'adminName' : ''}
        # str = json.dumps(data, sort_keys=True, indent=4, separators=(',', ': ')
        str = json.dumps(data)
        print('json dumps:%s' % str)
        SingletonTcpSocket().sock.send(bytes(str, encoding="utf8"))
        ...
        # 解析json数据
        resData = json.loads(str.decode('ascii'))
